...
const ServiceRequestDetails = () => {
  ...
  const statusInfo =
    statusConfig[request.status?.toLowerCase()] || statusConfig.pending;
  ...
  const showMessagingButton =
    loggedInUserRole === "mechanic" && !isCustomerViewingOwnRequest;
  ...
  const handleAcceptRequest = async () => {
    if (loggedInUserRole !== "mechanic") {
      Swal.fire({
        ...
        text: "Only a mechanic can accept this request.",
        confirmButtonColor: "#f97316",
      });
      return;
    }
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium border ${statusInfo.color} whitespace-nowrap`}
                >
          ...
          <div className="lg:col-span-2 space-y-6">
            <InfoCard title="Service Information" icon={Wrench}>
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <DetailItem
                    label="Brand"
                    value={request.serviceDetails?.vehicleInfo?.brand}
                  />
                </div>
                <div className="space-y-2">
                  <DetailItem
                    label="Model"
                    value={request.serviceDetails?.vehicleInfo?.model}
                  />
                  <DetailItem
                    label="Year"
                    value={request.serviceDetails?.vehicleInfo?.year}
                  />
                  <DetailItem
                    label="VIN"
                    value={request.serviceDetails?.vehicleInfo?.vin}
                  />
                </div>
              </div>
            </InfoCard>
            ...
            <InfoCard title="Problem Details" icon={AlertTriangle}>
              ...
              <div className="space-y-4">
                <DetailItem
                  label="Problem Title"
                  value={request.serviceDetails?.problemTitle}
                  largeValue
                />
                <div>
                  ...
                  <p className="text-orange-950 bg-orange-100 p-4 rounded-lg border border-primary shadow-inner text-sm leading-relaxed">
                    {request.serviceDetails?.description ||
                      "No detailed description provided by the customer."}
                  </p>
                </div>
                ...
                {request.serviceDetails?.images?.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-3">
                      Problem Images ({request.serviceDetails.images.length})
                    </label>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                      {request.serviceDetails.images.map((img, index) => (
                        <div
                          key={index}
                          className="relative aspect-square cursor-pointer overflow-hidden rounded-lg group shadow-sm hover:shadow-md transition-shadow"
                        >
                          <img
                            src={img}
                            ...
                            onError={(e) => {
                              ...
                              e.target.parentNode.innerHTML =
                                '<div class="w-full h-full flex items-center justify-center bg-gray-200 text-gray-400"><ImageIcon size={18} /></div>';
                            }}
          ...
          <div className="space-y-6">
            <InfoCard title="Customer Information" icon={User}>
              <div className="space-y-4">
                ...
                <DetailItem
                  label="Full Name"
                  value={request?.userName || "Not Provided"}
                />
                <DetailItem
                  label="Email"
                  value={request.user?.email || request.userEmail}
                  icon={Mail}
                />
            ...
            <InfoCard title="Schedule & Budget" icon={CalendarClock}>
              <div className="space-y-3">
                ...
                <DetailItem
                  label="Time Slot"
                  value={
                    request.preferredSchedule?.timeSlot
                      ? request.preferredSchedule.timeSlot
                          .charAt(0)
                          .toUpperCase() +
                        request.preferredSchedule.timeSlot.slice(1)
                      : "Any time"
                  }
                />
                <DetailItem
                  label="Flexibility"
                  value={
                    request.preferredSchedule?.flexibility
                      ? request.preferredSchedule.flexibility
                          .charAt(0)
                          .toUpperCase() +
                        request.preferredSchedule.flexibility.slice(1)
                      : "Flexible"
                  }
                />
    ...
    <div className="flex items-start gap-2">
      ...
      <span
        className={`${capitalize ? "capitalize" : ""} ${
          largeValue ? "break-words text-base font-medium" : "text-sm"
        } leading-tight`}
      >
  ...
  return (
    <div className="flex gap-3 relative">
      <div className="flex flex-col items-center">
        <div
          className={`w-3 h-3 rounded-full ${color} ring-4 ${
            active ? "ring-orange-100" : "ring-gray-100"
          } z-10`}
        />
        <div
          className={`w-0.5 h-full ${
            pending ? "bg-gray-200" : "bg-orange-200"
          } mt-1 -mb-2`}
        />
      </div>
