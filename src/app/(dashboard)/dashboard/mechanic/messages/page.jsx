...
export default function MechanicChatPage() {
    ...
    const debouncedStopTyping = useDebouncedCallback(() => {
        if (socket && activeConversation?._id) {
            // Note: The socket event `stopTyping` should include the user ID to ensure
            // the receiver only stops the *correct* indicator. I'll add the user ID here
            // based on the previous working example, even though the provided boilerplate didn't use it.
    ...
    useEffect(() => {
                    ...
                    const updatedConvs = prev.map(conv =>
                        conv._id === msg.chatId
                            ? {
                                ...
                                lastMessageAt: msg.createdAt, // Use the fixed message time
                            }
                            : conv
                    );
        ...
        // Note: The server code broadcasts 'typing' with only chatId. 
        // We handle it here, assuming any 'typing' event in this chat is from the other user.
    ...
    const handleSend = async () => {
        ...
        // Optimistic message with temporary ID and current time
        ...
        // 3. API Call (The API server is responsible for setting the final, definitive time)
        try {
            ...
            // 4. Emit to socket (Send the message with the client-generated 'now' time for quick display on the receiver's end)
                    ...
                    <div className="w-full lg:w-1/3 xl:w-1/4 flex flex-col overflow-y-auto border-r border-gray-100 flex-shrink-0 transition-all duration-300 ease-in-out">
                        ...
                        {isLoading ? (
                            ...
                            <div className="p-10 text-center text-gray-400">
                                ...
                                <p className="text-sm mt-1">Users will appear here when they contact you.</p>
                            </div>
                        ) : (
                ...
                {(activeConversation && (showChat || !isMobileDevice)) ? (
                    ...
                    <div className={`hidden lg:flex flex-1 items-center justify-center bg-gray-50/50`}>
                        <div className="text-center text-gray-500 p-10">
                            ...
                            <p className="mt-2 text-sm">Your chats with customers will appear here.</p>
                        </div>
                    </div>
                )}
            </div>
