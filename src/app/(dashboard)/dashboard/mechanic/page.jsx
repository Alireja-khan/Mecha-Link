...
import { Users, Store, Wrench, UserCheck, DollarSign, Star, Shield, Crown, Calendar, Eye, FileText, Settings, BarChart3, Clock, CheckCircle, XCircle, AlertTriangle, MapPin, Phone, Mail } from "lucide-react";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, PieChart, Pie, Cell, ResponsiveContainer, Legend, BarChart, Bar } from "recharts";
...
const MechanicDashboardOverview = () => {
  ...
  try {
    ...
    const completedRequests = requestsArray.filter(req => req.status === 'completed').length;
    const inProgressRequests = requestsArray.filter(req => req.status === 'in-progress').length;
    ...
    const totalEarnings = requestsArray
      ...
      .reduce((sum, req) => {
        const cost = req.serviceDetails?.estimatedCost || 
                   req.estimatedCost || 
    ...
    const generateMonthlyRevenue = (requests) => {
        ...
        requests
            .filter(req => req.status === 'completed')
            .forEach(req => {
                const date = req.updatedAt || req.createdAt || req.requestedDate || new Date();
                ...
                const cost = req.serviceDetails?.estimatedCost || 
                           ...
                           req.cost || 
                           0;
                revenueByMonth[monthName] = (revenueByMonth[monthName] || 0) + (parseFloat(cost) || 0);
            });
    ...
    const mechanicMetrics = [
        ...
        },
        {
            id: 4,
            title: "Total Revenue",
            value: loading ? "…" : `$${performanceStats.totalEarnings?.toLocaleString() || 0}`,
            change: "+18%",
            trend: "up",
            icon: DollarSign,
            color: "emerald",
        ...
        },
        {
            id: 6,
            title: "Shop Rating",
            value: loading ? "…" : performanceStats.averageRating > 0 ? performanceStats.averageRating : "0.0",
            change: "+0.3",
            trend: "up",
            icon: Star,
            color: "orange",
    ...
    const serviceTypeData = Object.entries(serviceTypeBreakdown).map(([name, value]) => ({
        name,
        value,
    }));

    // Recent activity data
    const recentActivity = acceptedRequests.slice(-5).reverse().map(request => ({
        id: request._id,
        customer: request.user?.name || request.user?.userName || request.userEmail || "Unknown Customer",
        service: request.deviceType || request.problemCategory || "General Service",
        status: request.status || "pending",
        date: new Date(request.updatedAt || request.createdAt || request.requestedDate || Date.now()).toLocaleDateString(),
        ...
        <div className="min-h-screen p-4 md:p-8 bg-base-200 space-y-8">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <div className="flex items-center gap-2 bg-white/20 px-4 py-2 rounded-xl">
                            <MapPin size={16} />
                            <span className="text-sm">{shopData?.shop?.address?.city || "City"}</span>
                        </div>
                        <div className="flex items-center gap-2 bg-white/20 px-4 py-2 rounded-xl">
                            <Phone size={16} />
                            <span className="text-sm">{shopData?.shop?.contact?.phone || "N/A"}</span>
                        </div>
                        <div className="flex items-center gap-2 bg-white/20 px-4 py-2 rounded-xl">
                            <Star size={16} />
                            <span className="text-sm">Rating: {performanceStats.averageRating || "0.0"}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 bg-base-100 rounded-3xl p-6 md:p-8 border border-neutral shadow-xl transition-all duration-300 hover:shadow-2xl">
                    <h2 className="text-xl md:text-2xl font-bold text-base-content mb-8">Monthly Revenue</h2>
                ...
                <div className="bg-base-100 rounded-3xl p-6 md:p-8 border border-neutral shadow-xl transition-all duration-300 hover:shadow-2xl">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
                            <Pie
                                data={serviceTypeData.length > 0 ? serviceTypeData : [{name: "No Data", value: 1}]}
                                dataKey="value"
                                nameKey="name"
                                cx="50%"
                                cy="50%"
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            recentActivity.map((activity, i) => (
                                <div key={i} className="flex items-center justify-between p-4 bg-base-200 rounded-xl border border-base-300 hover:bg-base-300 transition-colors duration-200">
                                    <div className="flex items-center gap-4 flex-1">
                                        <div className={`w-3 h-3 rounded-full ${urgencyColors[activity.urgency] || 'bg-gray-500'}`}></div>
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-base-content truncate">{activity.customer}</p>
                                            <p className="text-sm text-base-content/60">{activity.service}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusColors[activity.status] || 'bg-gray-100 text-gray-600'}`}>
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            recentReviews.map((review, i) => (
                                <div key={i} className="p-4 bg-base-200 rounded-xl border border-base-300 hover:bg-base-300 transition-colors duration-200">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 bg-gradient-to-br from-primary to-orange-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                                {review.userName?.charAt(0) || review.customerName?.charAt(0) || "C"}
                                            </div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span className="text-sm font-bold text-base-content">{review.rating || "0"}.0</span>
                                        </div>
                                    </div>
                                    <p className="text-sm text-base-content/70 line-clamp-2">{review.feedback || review.comment || "No comment provided"}</p>
                            ...
                            <div className="text-center py-8 text-base-content/70">No recent reviews</div>
                        )}
                    </div>
                </div>
            </div>
            ...
            {/* Shop Information */}
            <div className="bg-base-100 rounded-3xl p-6 md:p-8 border border-neutral shadow-xl transition-all duration-300 hover:shadow-2xl">
                <h2 className="text-xl sm:text-2xl font-bold text-base-content mb-6">Shop Information</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="space-y-2">
                        <h3 className="font-semibold text-base-content/70 text-sm">Shop Name</h3>
                        <p className="text-base-content font-medium">{shopData?.shop?.shopName || "N/A"}</p>
                    </div>
                    <div className="space-y-2">
                        <h3 className="font-semibold text-base-content/70 text-sm">Contact Email</h3>
                        <p className="text-base-content font-medium">{shopData?.shop?.contact?.businessEmail || "N/A"}</p>
                    </div>
                    <div className="space-y-2">
                        <h3 className="font-semibold text-base-content/70 text-sm">Phone</h3>
                        <p className="text-base-content font-medium">{shopData?.shop?.contact?.phone || "N/A"}</p>
                    </div>
                    <div className="space-y-2">
                        <h3 className="font-semibold text-base-content/70 text-sm">Status</h3>
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${shopData?.status === 'approved' ? 'bg-green-100 text-green-600' : shopData?.status === 'rejected' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600'}`}>
