...
const MechanicProfile = ({shopId}) => {
  ...
  const fetchAds = async () => {
    ...
    try {
      const adsRes = await fetch(`/api/ads?email=${loggedInUser.email}`);
      ...
      <div className="flex items-center justify-center min-h-screen bg-base-200">
        <div className="text-center p-8 max-w-md">
          ...
          <p className="text-base-content/70 mb-6">
            We couldn't find a shop associated with your account. Would you like
            to create one?
          </p>
  ...
  const processedShopData = {
    ...
    // Business Details
    description:
      shopData?.shop?.details || "Professional automotive repair services",
    ...
    joinedDate: shopData?.createdAt
      ...
      : "N/A",
    approvedDate: shopData?.approvedAt
      ? new Date(shopData.approvedAt).toLocaleDateString()
      : null,
    logo: shopData?.shop.logo,

    // Performance Metrics
    metrics: {
      totalServices: Object.values(shopData?.vehicleTypes || {}).reduce(
        (total, category) =>
          total + Object.values(category || {}).flat().length,
        0
      ),
    ...
    return (
      <div className="bg-base-100 rounded-2xl p-6 border border-neutral/40 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] group">
        <div className="flex items-center justify-between mb-4">
          <div
            className={`p-3 rounded-xl ${selectedColor.bg} ${selectedColor.hoverBg} transition-colors duration-300`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {Object.entries(services).map(([serviceType, serviceList]) => (
          <div key={serviceType} className="space-y-2">
            <h5 className="font-semibold text-base-content text-sm">
              {serviceType}
            </h5>
          ...
          <div className="lg:col-span-2 bg-base-100 rounded-3xl p-6 md:p-8 border border-neutral/40 shadow-xl">
            ...
            {processedShopData.certifications.length > 0 && (
              <div className="bg-base-100 rounded-3xl p-6 mt-5 border border-neutral/40">
                <h2 className="text-xl font-extrabold text-base-content mb-4">
                  Certification
                </h2>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {activeTab === "services" && (
              <div className="bg-base-100 rounded-3xl p-6 md:p-8 border border-neutral/40 shadow-xl">
                ...
                <div className="space-y-6">
                  {Object.entries(processedShopData.services).map(
                    ([category, services]) => (
                      <ServiceCategory
                        key={category}
                        category={category}
                    ...
                    {[
                      {
                        icon: Phone,
                        label: "Phone",
                        value: processedShopData.phone,
                      },
                      {
                        icon: Mail,
                        label: "Email",
                        value: processedShopData.email,
                      },
                      {
                        icon: Clock,
                        label: "Working Hours",
                        value: `${processedShopData.workingHours.open} - ${processedShopData.workingHours.close}`,
                      },
                      {
                        icon: Calendar,
                        label: "Weekend",
                        value: processedShopData.workingHours.weekend,
                      },
                    ].map((item, index) => (
                      <div
                        key={index}
                    ...
                    <div className="bg-base-200 rounded-xl p-6 border border-base-300 h-full">
                      ...
                      <div className="space-y-3">
                        <p className="text-base-content font-medium">
                          {processedShopData.address.street}
                        </p>
                        <div className="text-base-content/70 text-sm space-y-1">
                          <p>
                            {processedShopData.address.city},{" "}
                            {processedShopData.address.district}
                          </p>
                          <p>
                            {processedShopData.address.division},{" "}
                            {processedShopData.address.country}
                          </p>
                          <p>
                            Postal Code: {processedShopData.address.postalCode}
                          </p>
                        </div>
                        <button
                          onClick={() =>
                            window.open(
                              processedShopData.address.mapUrl,
                              "_blank"
                            )
                          }
            ...
            <div className="bg-base-100 rounded-3xl p-6 border border-neutral/40 shadow-xl">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div
                    key={day}
                    className="flex justify-between items-center p-3 bg-base-200 rounded-xl border border-base-300 transition-colors duration-200 hover:bg-base-300"
                  >
