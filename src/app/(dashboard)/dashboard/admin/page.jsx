...
import {
    Users, Store, Wrench, UserCheck, DollarSign, Star,
    Shield, Crown, Calendar, Eye, FileText, Settings, BarChart3
} from "lucide-react";
import {
...
const getColorClasses = (color) => {
    switch (color) {
        case 'orange':
            return { text: 'text-primary', bg_light: 'bg-primary/20 group-hover:bg-primary/30' };
        case 'blue':
            return { text: 'text-info', bg_light: 'bg-info/20 group-hover:bg-info/30' };
        case 'green':
            return { text: 'text-success', bg_light: 'bg-success/20 group-hover:bg-success/30' };
        case 'purple':
            return { text: 'text-purple-500', bg_light: 'bg-purple-500/20 group-hover:bg-purple-500/30' };
        case 'emerald':
            return { text: 'text-success', bg_light: 'bg-success/20 group-hover:bg-success/30' };
        case 'yellow':
            return { text: 'text-warning', bg_light: 'bg-warning/20 group-hover:bg-warning/30' };
        default:
            return { text: 'text-base-content', bg_light: 'bg-base-200/50 group-hover:bg-base-200' };
    }
};
...
const AdminDashboardOverview = () => {
    ...
    const calculateTotalMechanics = (shopsArray) => {
        ...
        return shopsArray.reduce((total, shop) => total + (shop.shop?.mechanicCount || 0), 0);
    };
    ...
    const calculateTotalRatings = (reviewsArray) => {
        ...
        return reviewsArray.reduce((total, review) => total + (review.rating || 0), 0);
    };
    ...
    const serviceTypeBreakdown = Object.entries(serviceTypeCount).map(([name, value]) => ({ name, value }));
    ...
    const StatCard = ({ icon: Icon, value, label, change, trend, color = "orange" }) => {
        ...
        return (
            <div className="bg-base-100 rounded-2xl p-5 sm:p-6 border border-neutral/40 shadow-lg hover:shadow-xl transition-all duration-300 hover:sm:scale-[1.02] group">
                <div className="flex items-start justify-between mb-3 sm:mb-4">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${trend === 'up' ? 'bg-success/20 text-success' : 'bg-error/20 text-error'}`}>
                        {trend === 'up' ? '↑' : '↓'} {change}
                    </span>
                </div>
    ...
    return (
        <div className="min-h-screen w-full overflow-x-hidden overflow-y-visible p-4 sm:p-6 bg-base-200 space-y-6 sm:space-y-8 mx-auto text-base-content">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-primary-content/90 text-sm sm:text-lg mt-1 sm:mt-2">
                                Welcome back, <span className="font-semibold">{loggedInUser.name}</span>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-6 w-full overflow-hidden">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
                <div className="xl:col-span-2 bg-base-100 rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-neutral/40 shadow-xl transition-all duration-300 hover:shadow-2xl hover:sm:scale-[1.005]">
                    <h2 className="text-xl sm:text-2xl font-bold text-base-content mb-6 sm:mb-8">Service Requests Trend</h2>
                    <ResponsiveContainer width="100%" height={250}>
                        <LineChart data={serviceTrends} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                            ...
                            <Tooltip contentStyle={{ borderRadius: '8px', fontSize: '12px', background: 'var(--base-300, #ffffff)', border: '1px solid var(--neutral, #dddddd)', color: 'var(--base-content, #343434)' }} />
                            <Line type="monotone" dataKey="requests" stroke="var(--color-primary)" strokeWidth={3} dot={{ fill: 'var(--color-primary)', r: 4 }} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>

                <div className="bg-base-100 rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-neutral/40 shadow-xl transition-all duration-300 hover:shadow-2xl hover:sm:scale-[1.005]">
                    <h2 className="text-xl sm:text-2xl font-bold text-base-content mb-6 sm:mb-8">Service Type Breakdown</h2>
                    <ResponsiveContainer width="100%" height={250}>
                        <PieChart>
                            <Pie data={serviceTypeBreakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={renderCustomizedLabel} labelLine={false}>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
                <div className="bg-base-100 col-span-1 xl:col-span-2 rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-neutral/40 shadow-xl transition-all duration-300 hover:shadow-2xl hover:sm:scale-[1.005]">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                        {loading ? (
                            ...
                            recentServiceReq.map((req, i) => (
                                <div key={i} className="flex items-center justify-between p-3 sm:p-4 bg-base-200 rounded-xl border border-base-300 transition-colors duration-200 hover:bg-base-300 overflow-hidden">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                        {loading ? (
                            ...
                            recentSignups.map((user, i) => (
                                <div key={i} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-base-200 rounded-xl border border-base-300 transition-colors duration-200 hover:bg-base-300 overflow-hidden">
                                    <div className="w-10 h-10 bg-gradient-to-br from-primary to-secondary rounded-lg sm:rounded-xl flex items-center justify-center text-primary-content font-bold text-sm">
                ...
                <div className="space-y-3 sm:space-y-4">
                    {loading ? (
                        ...
                        recentShopsData.map((shop, i) => (
                            <div key={i} className="flex items-center justify-between p-3 sm:p-4 bg-base-200 rounded-xl border border-base-300 transition-colors duration-200 hover:bg-base-300 overflow-hidden">
                                <div className="w-10 h-10 mr-2 bg-gradient-to-br from-primary to-secondary rounded-lg sm:rounded-xl flex items-center justify-center text-primary-content font-bold text-sm">
