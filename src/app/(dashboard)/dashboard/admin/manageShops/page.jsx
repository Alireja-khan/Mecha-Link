...
const ManageShops = () => {
    ...
    const getStatusBadge = (status) => {
        ...
        switch (status) {
            case "approved":
                return <span className={`${base} bg-success/10 text-success border-success/30`}>Approved</span>;
            case "pending":
            ...
            <div className="bg-base-100 p-4 rounded-xl border border-neutral/50 shadow-lg hover:shadow-xl transition-all duration-200">
                <div className="flex items-start gap-3 mb-3 border-b border-neutral/50 pb-3">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="font-semibold text-base-content truncate">{shopDetails.shopName || 'Unknown Shop'}</p>
                        <p className="text-xs text-base-content/70 truncate flex items-center gap-1"><User size={12} className="text-base-content/40" />{shopDetails.ownerName || 'Unknown Owner'}</p>
                        <p className="text-xs text-base-content/70 truncate flex items-center gap-1"><MapPin size={12} className="text-base-content/40" />{address.city || 'Location N/A'}</p>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 flex-wrap gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-xs text-base-content/60 flex items-center gap-1"><Clock size={12} />{formatDateShort(shop.createdAt)}</span>
                    </div>
    ...
    return (
        <div className="min-h-screen w-full p-3 sm:p-4 lg:p-6 mx-auto bg-base-200">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-4 sm:mb-6 lg:mb-8">
                <StatCard icon={Store} value={stats.total} label="Total Shops" color="primary" />
                <StatCard icon={Clock} value={stats.pending} label="Pending Approval" color="warning" />
                <StatCard icon={Check} value={stats.approved} label="Approved Shops" color="success" />
            ...
            <div className="bg-base-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-neutral shadow-2xl">
                ...
                <div className="block xl:hidden space-y-4">
                    {filteredShops.length > 0 ? filteredShops.map(s => <ShopMobileCard key={s._id} shop={s} />) : <div className="text-center py-12"><MessageSquare size={48} className="mx-auto text-base-content/30" /><p className="text-base-content/60">No shops found</p></div>}
                </div>
                    ...
                    <table className="min-w-full divide-y divide-neutral">
                        <thead className="bg-base-300">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-sm font-semibold text-base-content">Categories</th>
                                    ...
                                    <tr key={shop._id} className="hover:bg-base-200/50 transition-colors duration-200">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-base-content">{shop.shop?.shopName || "N/A"}</p>
                                                    <button
                                                        onClick={() => openDetailModal(shop)}
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm text-base-content">{shop.shop?.ownerName || "N/A"}</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Mail size={14} className="text-primary" />
                                                    <span className="text-sm text-base-content/70">{shop.shop?.ownerEmail || "N/A"}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1">
                                                {Array.isArray(shop.shop?.categories) ? (
                                                    ...
                                                    <span className="text-base-content/70 text-sm">N/A</span>
                                                )}
                                ...
                                <tr>
                                    <td colSpan="6" className="text-center py-12">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-base-content/60 text-lg">No shops found</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                ...
                <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md bg-base-content/10 z-50 p-4">
                    <div className="bg-base-100 rounded-3xl p-6 sm:p-8 w-full max-w-4xl border border-neutral shadow-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl sm:text-2xl font-bold text-base-content">Shop Details</h2>
                            <button
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-primary/10 rounded-xl border border-primary/30">
                                ...
                                <div>
                                    <h3 className="text-lg sm:text-xl font-bold text-base-content">{selectedShop.shop?.shopName || "N/A"}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                <div className="p-4 bg-info/10 rounded-xl border border-info/30">
                                    <h5 className="font-semibold text-info mb-2">Shop Information</h5>
                                    <div className="space-y-2 text-sm text-base-content">
                                        <p><strong>Name:</strong> {selectedShop.shop?.shopName}</p>
                                        <p><strong>Description:</strong> {selectedShop.shop?.details || "No description"}</p>
                                        <p><strong>Categories:</strong> {Array.isArray(selectedShop.shop?.categories) ? selectedShop.shop.categories.join(", ") : "No categories"}</p>
                                        <p><strong>Mechanics:</strong> {selectedShop.shop?.mechanicCount || "Not specified"}</p>
                                    </div>
                                </div>

                                <div className="p-4 bg-success/10 rounded-xl border border-success/30">
                                    <h5 className="font-semibold text-success mb-2">Owner Information</h5>
                                    <div className="space-y-2 text-sm text-base-content">
                                        <p><strong>Name:</strong> {selectedShop.shop?.ownerName || "N/A"}</p>
                                        <p><strong>Email:</strong> {selectedShop.shop?.ownerEmail || "N/A"}</p>
                                        <p><strong>Phone:</strong> {selectedShop.shop?.contact?.phone || "Not provided"}</p>
                                        <p><strong>Business Email:</strong> {selectedShop.shop?.contact?.businessEmail || "Not provided"}</p>
                                    </div>
                                </div>

                                <div className="p-4 bg-secondary/10 rounded-xl border border-secondary/30">
                                    <h5 className="font-semibold text-secondary mb-2">Location</h5>
                                    <div className="space-y-2 text-sm text-base-content">
                                        <p><strong>Street:</strong> {selectedShop.shop?.address?.street || "Not provided"}</p>
                                        <p><strong>City:</strong> {selectedShop.shop?.address?.city || "Not provided"}</p>
                                        <p><strong>Country:</strong> {selectedShop.shop?.address?.country || "Not provided"}</p>
                                        <p><strong>Postal Code:</strong> {selectedShop.shop?.address?.postalCode || "Not provided"}</p>
                                        {selectedShop.shop?.location && (
                                            <>
                                                <p><strong>Latitude:</strong> {selectedShop.shop.location.latitude || "Not set"}</p>
                                                <p><strong>Longitude:</strong> {selectedShop.shop.location.longitude || "Not set"}</p>
                                            </>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="p-4 bg-base-200 rounded-xl border border-neutral">
                                    <h5 className="font-semibold text-base-content mb-2">Status Information</h5>
                                    <div className="space-y-2 text-sm text-base-content/90">
                                        ...
                                        {selectedShop.status === 'rejected' && selectedShop.rejectionReason && (
                                            <p className="text-error border-t border-error/30 pt-2"><strong>Reason:</strong> {selectedShop.rejectionReason}</p>
                                        )}
                                        {selectedShop.approvedAt && (
                                            <p className="text-success">Approved on: {formatDate(selectedShop.approvedAt)}</p>
                                        )}
                                        <p className="text-base-content/60">ID: <span className="text-xs break-all">{selectedShop._id}</span></p>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {Object.entries(selectedShop.shop.vehicleTypes).map(([vehicleType, categories]) => (
                                            <div key={vehicleType} className="border border-neutral rounded-lg p-3 bg-base-100">
                                                <h6 className="font-bold text-base text-primary mb-2 border-b pb-1">{vehicleType}</h6>
                                                <div className="space-y-2">
                                                    {Object.entries(categories).map(([category, services]) => (
                                                        <div key={category}>
                                                            <p className="text-sm font-semibold text-base-content">{category}:</p>
                                                            <div className="flex flex-wrap gap-1 mt-1">
                                                                {services.map((service, index) => (
                                                                    <span key={index} className="px-2 py-1 bg-base-300/40 text-base-content/80 text-xs rounded border border-neutral/20">
                                                                        {service}
                                                                    </span>
                                                                ))}
                                                            </div>
