...
import { Bell, CheckCircle, ClipboardList, Gift, Trash2, Eye, Archive } from "lucide-react";
      ...
      <div className="space-y-2 p-3">
        {notifications.length === 0 ? (
          <div className="text-center py-12 transition-all duration-300">
            <Bell className="mx-auto text-base-content/30 mb-3 transition-transform duration-300" size={48} />
            <p className="text-base-content/60 font-medium transition-colors duration-300">No notifications</p>
            <p className="text-sm text-base-content/40 mt-1 transition-colors duration-300">You're all caught up!</p>
          </div>
        ) : (
          notifications.map((notif, idx) => (
            ...
            >
              <div className="flex items-start gap-4">
                {/* Icon */}
                <div className={`p-2 rounded-xl transition-all duration-300 ${getNotificationBadge(notif.type)} border`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize transition-all duration-300 ${getNotificationBadge(notif.type)}`}>
                      {notif.type || 'general'}
                    </span>
                    {!notif.read && (
